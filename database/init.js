...
CREATE TABLE IF NOT EXISTS orders (
  ...
  payment_method TEXT, paid BOOLEAN NOT NULL DEFAULT FALSE, due_date DATE, notes TEXT, status TEXT NOT NULL DEFAULT 'ORCAMENTO', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), deleted_at TIMESTAMPTZ
);
...
async function initDb() {
  ...
  await p.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS paid BOOLEAN NOT NULL DEFAULT FALSE`);
  await p.query(`UPDATE orders SET paid=TRUE WHERE paid=FALSE AND status='CONFIRMADO' AND EXISTS (SELECT 1 FROM transactions t WHERE t.reference_type='order' AND t.reference_id=orders.id AND t.type='RECEITA' AND t.paid=TRUE AND t.deleted_at IS NULL)`);
  ...
  await p.query(`CREATE TABLE IF NOT EXISTS shipments (id BIGSERIAL PRIMARY KEY, order_id BIGINT NOT NULL REFERENCES orders(id) ON DELETE RESTRICT, customer_id BIGINT REFERENCES customers(id) ON DELETE SET NULL, street TEXT, number TEXT, complement TEXT, neighborhood TEXT, city TEXT, state TEXT, postal_code TEXT, shipping_method TEXT NOT NULL DEFAULT 'OUTRO', freight NUMERIC(14,2), estimated_delivery DATE, delivered_at DATE, status TEXT NOT NULL DEFAULT 'PENDENTE', tracking_code TEXT, tracking_url TEXT, notes TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), deleted_at TIMESTAMPTZ)`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS customer_id BIGINT REFERENCES customers(id) ON DELETE SET NULL`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS street TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS number TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS complement TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS neighborhood TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS city TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS state TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS postal_code TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS shipping_method TEXT NOT NULL DEFAULT 'OUTRO'`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS freight NUMERIC(14,2)`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS estimated_delivery DATE`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS delivered_at DATE`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'PENDENTE'`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS tracking_code TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS tracking_url TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS notes TEXT`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`);
  await p.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ`);
  await p.query(`UPDATE shipments SET shipping_method='OUTRO' WHERE shipping_method IS NULL`);
  await p.query(`UPDATE shipments SET status='PENDENTE' WHERE status IS NULL`);
