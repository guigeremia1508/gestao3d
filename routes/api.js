...
router.put('/orders/:id', adminOnly, async (req,res,next)=>{
  try {
    ...
    let freight=null;
    if(b.freight!==''&&b.freight!=null){freight=Number(b.freight);if(!Number.isFinite(freight)||freight<0)return res.status(400).json({error:'Frete inválido'});}
    const total=calculateOrderTotal(qty,up,disc,freight||0);
    ...
    const result=await withTransaction(async tx=>{
      ...
      const validStatuses=['ORCAMENTO','AGUARDANDO_PAGAMENTO','CONFIRMADO','EM_PRODUCAO','ACABAMENTO','PRONTO','ENVIADO','ENTREGUE','CANCELADO'];
      if(!validStatuses.includes(nextStatus))throw Object.assign(new Error('Status de pedido inválido'),{status:400});
      ...
      const nextPaid=Object.prototype.hasOwnProperty.call(b,'paid')?bool(b.paid):Boolean(old.paid);
      await tx.run('UPDATE orders SET customer_id=$1,product_id=$2,quantity=$3,material=$4,roll_id=$5,unit_price=$6,discount=$7,freight=$8,total=$9,payment_method=$10,paid=$11,due_date=$12,notes=$13,status=$14 WHERE id=$15',[nextCustomer,nextProduct,qty,material,rollId||null,up,disc,freight,total,b.payment_method||null,nextPaid,b.due_date||null,b.notes||null,nextStatus,req.params.id]);
      ...
      const wasPaid=Boolean(receipt?.paid)||Boolean(old.paid);

      if(nextStatus==='CANCELADO'){
        if(wasPaid && !refund){
          await tx.run(`INSERT INTO transactions(type,category,description,amount,date,reference_id,reference_type,paid,paid_at,notes) VALUES('RECEITA','Estorno',$1,$2,$3,$4,'order_refund',TRUE,NOW(),$5)`,[`Estorno do pedido #${req.params.id}`,-Math.abs(n(receipt?.amount||total)),today(),req.params.id,'Cancelamento do pedido']);
        }
      } else if(nextPaid){
        ...
        else await tx.run('UPDATE transactions SET amount=$1,paid=TRUE,paid_at=COALESCE(paid_at,NOW()),deleted_at=NULL WHERE id=$2',[total,receipt.id]);
      } else if(wasPaid && receipt?.paid && !refund){
        await tx.run(`INSERT INTO transactions(type,category,description,amount,date,reference_id,reference_type,paid,paid_at,notes) VALUES('RECEITA','Estorno',$1,$2,$3,$4,'order_refund',TRUE,NOW(),$5)`,[`Estorno do pagamento do pedido #${req.params.id}`,-Math.abs(n(receipt.amount)),today(),req.params.id,'Pagamento desmarcado']);
      }
      if(nextPaid && nextStatus!=='CANCELADO' && receipt && !refund) await tx.run('UPDATE transactions SET amount=$1,paid=TRUE,paid_at=COALESCE(paid_at,NOW()) WHERE id=$2',[total,receipt.id]);

      // Keep production in sync with the order status.
      if(['CONFIRMADO','EM_PRODUCAO','ACABAMENTO','PRONTO','ENTREGUE'].includes(nextStatus)){
        let prodJob=await tx.get("SELECT * FROM production_jobs WHERE order_id=$1 AND status<>'CANCELADO' ORDER BY id ASC LIMIT 1 FOR UPDATE",[req.params.id]);
        if(!prodJob){
          const prod=await tx.get('SELECT * FROM products WHERE id=$1 AND deleted_at IS NULL',[nextProduct]);
          if(!prod)throw Object.assign(new Error('Produto não encontrado'),{status:404});
          prodJob=await tx.get('INSERT INTO production_jobs(order_id,project_id,version_id,product_id,roll_id,status,est_weight_g,est_time_min) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',[req.params.id,prod.project_id||null,prod.version_id||null,nextProduct,rollId||prod.material_roll_id||null,'AGUARDANDO',n(prod.weight_g)*qty,n(prod.print_time_min)*qty]);
        }
        const prodStatus=nextStatus==='ENTREGUE'?'ENTREGUE':nextStatus==='PRONTO'?'PRONTO':nextStatus==='ACABAMENTO'?'ACABAMENTO':nextStatus==='EM_PRODUCAO'?'IMPRIMINDO':'AGUARDANDO';
        if(prodStatus!==prodJob.status) await tx.run('UPDATE production_jobs SET status=$1,finished_at=CASE WHEN $1 IN (\'PRONTO\',\'ENTREGUE\') THEN COALESCE(finished_at,NOW()) ELSE finished_at END WHERE id=$2',[prodStatus,prodJob.id]);
      }
      if(nextStatus==='CANCELADO') await tx.run("UPDATE production_jobs SET status='CANCELADO' WHERE order_id=$1 AND status<>'CANCELADO'",[req.params.id]);

      // ENVIADO always appears in Envios: create the shipment once, using the order freight.
      if(nextStatus==='ENVIADO'){
        const existing=await tx.get("SELECT id FROM shipments WHERE order_id=$1 AND deleted_at IS NULL AND status<>'CANCELADO' LIMIT 1 FOR UPDATE",[req.params.id]);
        if(!existing){
          const customer=await tx.get('SELECT city FROM customers WHERE id=$1',[nextCustomer]);
          await tx.run(`INSERT INTO shipments(order_id,customer_id,city,freight,status) VALUES($1,$2,$3,$4,'PENDENTE')`,[req.params.id,nextCustomer,customer?.city||null,freight]);
        }
      }
      if(nextStatus==='CANCELADO') await tx.run("UPDATE shipments SET status='CANCELADO',deleted_at=COALESCE(deleted_at,NOW()),updated_at=NOW() WHERE order_id=$1 AND deleted_at IS NULL",[req.params.id]);

      return {ok:true,paid:nextPaid,status:nextStatus};
    });
    res.json(result);
  } catch(e){next(e);}
});
