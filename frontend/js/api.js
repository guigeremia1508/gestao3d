const API = {
  ...
  async request(method,path,body){const headers={};if(body instanceof FormData){}else if(body!==undefined){headers['Content-Type']='application/json';}
    ...
    if(res.status===401 && !path.startsWith('/auth/me') && !path.startsWith('/auth/logout')){doLogout();return null;} const raw=await res.text();let data={};try{data=raw?JSON.parse(raw):{};}catch{throw new Error('Resposta inválida do servidor');} if(!res.ok)throw new Error(data.error||'Erro desconhecido');return data;},
