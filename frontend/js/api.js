const API = {
  ...
  async request(method,path,body){const headers={};if(body instanceof FormData){}else if(body!==undefined){headers['Content-Type']='application/json';}
    ...
    if(res.status===403 && mut && body instanceof FormData) { this.csrfToken=null; } const raw=await res.text();let data={};try{data=raw?JSON.parse(raw):{};}catch{throw new Error('Resposta inválida do servidor');} if(!res.ok){const msg=data.error||`Erro HTTP ${res.status}`;throw new Error(msg)}return data;},
