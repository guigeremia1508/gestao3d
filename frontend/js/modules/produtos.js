...
pageRenderers.produtos = async function () {
  ...
  const names = ['produtos','projetos','impressoras','rolos','peças','consumíveis'];
  const values = requests.map((r,i) => { if (r.status === 'fulfilled') return r.value; (window._produtoLoadErrors ||= []).push(`${names[i]}: ${r.reason?.message||'indisponível'}`); return []; });
  [_produtos, _projetosP, _printersP, _rollsP, _partsP, _consumablesP] = values;
  renderProdutos();
};

function renderProdutos(filter = '') {
  const list = filter ? _produtos.filter(p => (p.name+' '+(p.code||'')).toLowerCase().includes(filter)) : _produtos;
  const warning = window._produtoLoadErrors?.length ? `<div class="alert warn" style="margin-bottom:1rem">Alguns cadastros auxiliares não puderam ser carregados: ${g3dEscape(window._produtoLoadErrors.join(' | '))}. O restante da página continua disponível.</div>` : '';
  window._produtoLoadErrors = [];
  R('content').innerHTML = `${warning}
...
async function openProdutoModal(id) {
  ...
  try { _productComponents = id ? await API.get(`/products/${id}/components`) : []; } catch (e) { _productComponents = []; window._produtoLoadErrors = [`componentes: ${e.message}`]; }
  let cfg = window._productCfg || {};
  try { cfg = await API.get('/settings'); window._productCfg = cfg; } catch (e) { toast(`Configurações indisponíveis: ${e.message}`, 'err'); }
